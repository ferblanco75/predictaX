...
function statusLabel(status: string) {
  if (status === 'IN_PLAY' || status === 'PAUSED')
    return { text: 'En curso', color: 'text-green-600 dark:text-green-400' };
  if (status === 'FINISHED' || status === 'AWARDED')
    return { text: 'Finalizado', color: 'text-gray-400' };
...
export function FixtureCard({ fixture, compact = false }: FixtureCardProps) {
  ...
  return (
    <div
      className={`flex items-center gap-3 rounded-xl border border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 px-3 py-2.5 ${compact ? '' : 'shadow-sm'}`}
    >
      {/* Home */}
      <div className="flex items-center gap-1.5 flex-1 justify-end min-w-0">
        ...
        {home_team_crest && (
          <img src={home_team_crest} alt="" className="h-6 w-6 object-contain shrink-0" />
        )}
      </div>
      ...
      <div className="flex flex-col items-center min-w-[64px] shrink-0">
        ...
        {!isLive && label.text && <span className={`text-xs ${label.color}`}>{label.text}</span>}
      </div>

      {/* Away */}
      <div className="flex items-center gap-1.5 flex-1 justify-start min-w-0">
        {away_team_crest && (
          <img src={away_team_crest} alt="" className="h-6 w-6 object-contain shrink-0" />
        )}
