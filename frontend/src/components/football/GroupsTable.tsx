...
function SingleGroup({ standing, index }: { standing: Standing; index: number }) {
      ...
      <table className="w-full text-xs">
        ...
        <tbody>
          {standing.table.map((row, i) => (
            <tr
              key={`${standing.group}-${row.team_tla}`}
...
export function GroupsTable({ standings }: GroupsTableProps) {
  // Filter only real groups (have a group label) and sort A→L
  const groups = standings
    .filter((s) => s.group != null && s.table.length <= 6)
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
      {groups.map((standing, i) => (
        <SingleGroup key={standing.group ?? i} standing={standing} index={i} />
      ))}
    </div>
  );
}
